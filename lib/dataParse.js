...
const handleResults = (results) => {
  ...
  for (let i = 1; i < results.data.length; i++) {
    ...
    if (!(timeKey in filtered[stationKey][dateKey])) { filtered[stationKey][dateKey][timeKey] = {}; }
    if (!(turnstileKey in filtered[stationKey][dateKey][timeKey])) { filtered[stationKey][dateKey][timeKey][turnstileKey] = []; }
    filtered[stationKey][dateKey][timeKey][turnstileKey].push([results.data[i][7], results.data[i][3], results.data[i][9], results.data[i][10]]);
  }
