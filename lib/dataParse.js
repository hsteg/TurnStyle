...
const handleResults = (results) => {
  ...
  for (let i = 1; i < results.data.length; i++) {
    ...
    if (!(timeKey in filtered[stationKey][dateTimeKey])) { filtered[stationKey][dateTimeKey][timeKey] = {}; }
    if (!(turnstileKey in filtered[stationKey][dateTimeKey][timeKey])) { filtered[stationKey][dateTimeKey][timeKey][turnstileKey] = []; }
    filtered[stationKey][dateTimeKey][timeKey][turnstileKey].push([results.data[i][7], results.data[i][3], results.data[i][9], results.data[i][10]]);
  }
